 ...
 */ 
export class DrawPathSeg
{
    /**
     *  The SVG draw path command, this will be a single letter.
     */
   key : string;

   
   /**
    * The parsed parameters for the command.
    * Such as the command,
    * C x1 y1, x2 y2, x y
    *
    * Where the key would be 'C'
    * Each comma seperation would be an outer array
    *   e.g. x1 y1, x2 y2
    *        ^      ^
    * Each space seperation would be an inner array
    *   e.g. x1 y1
    *        ^  ^
    * Each value would be a number in the array.
    *   e.g. x1
    *        ^
    */
   items: Array<Array<number>>;

   constructor(key: string, items: Array<Array<number>>)
   {
       this.key = key;
       this.items = items;
   }
 ...
 */
 export class SVGUtils
 {
     ...
     */
    static GetNodeRefPt(node : Element) : Array<number> | null | undefined
    {
        ...
        {
            let xattr : Attr|null = node.attributes.getNamedItem("x");
            let yattr : Attr|null = node.attributes.getNamedItem("y");
        ...
        let cxattr : Attr|null = node.attributes.getNamedItem("cx");
        if(cxattr)
        {
            let cyattr : Attr|null = node.attributes.getNamedItem("cy");
            if(!cyattr)
                return [0, 0];
         ...
         
        {
            ...
            // a draw path.
            let dattr : Attr|null = node.attributes.getNamedItem("d");
            if(!dattr)
            return [0, 0];
      ...
      */
      static  MoveNode(node : Element, r2ofs : Array<number>)
     {
         if(!SVGUtils._validateNodePositionable(node))
             return;
         ...
         if( xattr)
         {
             let xpos: number = Number(xattr.value) + r2ofs[0];
         ...
         if( yattr)
         {
             let ypos: number = Number(yattr.value) + r2ofs[1];
         ...
         if( cxattr)
         {
             let xpos: number = Number(cxattr.value);
      ...
      */
      static  _validateNodePositionable(node: Element)
     {
         if(node.nodeName == 'g')
         {
     ...
     */
     static MoveNodes(nodes: Array<Element>, r2ofs: Array<number>)
     {
     ...
     */
     static  SplitD(dStr : string): Array<DrawPathSeg> 
     {
         // The return value will be an array that splits elements 
         // by instruction (the letters).
         //
         // 
         let ret: Array<DrawPathSeg> = new Array<DrawPathSeg>();

         let splits: Array<string> = dStr.split(' ');
         
         let last: DrawPathSeg | null = null;
         for(let i = 0; i < splits.length; ++i)
         {
             ...
             let vals: Array<string> = curStr.split(',');
             last?.items.push(vals.map(Number));
         }
         return ret;
     }
     ...
     */
     static MergeD(dCont: Array<DrawPathSeg>): string 
    {
        let str: string = "";
     ...
     */
     static  GetBoxDimension(svgDoc : XMLDocument) : Array<number>
    {
        let viewBoxAttr : string|null = svgDoc.documentElement.getAttribute("viewBox");
        if(!viewBoxAttr || viewBoxAttr?.length == 0)
            return [0, 0];

        let parts : Array<string> = viewBoxAttr.split(" ");
